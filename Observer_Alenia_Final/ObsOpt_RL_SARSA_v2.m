...
function RL = ObsOpt_RL_SARSA_v2(Nsearch,Niter)
    ...
    RL.E.domain_S = 0:RL.E.domain_S_Ts:1;
    RL.E.domain_S_dim = length(RL.E.domain_S);
    ...
    % Value function
    RL.S.V = zeros(1,RL.E.domain_S_dim);
    ...
    for s=1:RL.S.Nsearch
        ...
        % init state
        RL.S.state_pos = max(1,find(RL.E.domain_S >= nu,1,'first'));
        RL.S.state = RL.E.domain_S(RL.S.state_pos);
        % save setup
        RL.S.setup = setup;
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if RL.S.p_eps < RL.S.epsilon
            RL.S.action_pos = find(max(RL.S.Q(RL.S.state_pos,:)),1,'first');
            RL.S.A(:,1) = RL.A.domain_A(RL.S.action_pos);
        else
            cum_prob = cumsum(RL.S.search{s}.pi(RL.S.state_pos,:));
        ...
        for i=1:RL.S.Niter   
            ...
            %%% evaluate next state %%%
            % get last error
            nu = DynOpt.dJ_cond_story(4,end);
            % discretize state
            state_next_pos = find(RL.E.domain_S >= nu,1,'first');
            if isempty(state_next_pos)
                state_next_pos = RL.E.domain_S_dim;
            end
            % get new state value
            state_next = RL.E.domain_S(state_next_pos);
            % assign to struct
            RL.S.state_next_pos = state_next_pos;
            RL.S.state_next = state_next;

            %%% select action from policy - A'
            RL.S.p_eps = rand(1);
            RL.S.p(i) = rand(1);
            if RL.S.p_eps < RL.S.epsilon
                pos_1 = find(max(RL.S.Q(RL.S.state_next_pos,:)),1,'first');
                A_1 = RL.A.domain_A(pos_1);
            else
                cum_prob = cumsum(RL.S.search{s}.pi(RL.S.state_next_pos,:));
            ...
            if ~testing
                RL.S.Q(RL.S.state_pos,RL.S.action_pos) = RL.S.Q(RL.S.state_pos,RL.S.action_pos) + RL.S.alpha*(current_reward + RL.S.gamma*RL.S.Q(RL.S.state_next_pos,action_next_pos) - RL.S.Q(RL.S.state_pos,RL.S.action_pos));
            end
