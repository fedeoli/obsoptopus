%% Set input function
function [params_out,DynOpt] = set_input_v6(DynOpt,pos,x_att,x_pos,params)
    ...
    if DynOpt.params.input_flag == 1
        ...
        if DynOpt.recollect_input == 0 || DynOpt.optimise_input == 1
            ...
            % desired attitude
            temp_att = DynOpt.target_attitude + DynOpt.u_amp.*DynOpt.switch_pwm.*ones(n_act,1);
%             temp_att = min(pi/2, max(-pi/2, temp_att));

            % low pass filter
            if DynOpt.lowpass_pwm == 1
