%OBSERVER PARAMETERS SETTINGS (SAVED ON A .mat FILE) 

clear ObserverTest Agent

ObserverTest.Nagents = 4; %total number of agents
...
ObserverTest.ZeroErrors = 0; %to proceed with no measurement errors
...
ObserverTest.TrueInitialPositions = 0;%mainly for debug, assume that true positions are set initially
...
ObserverTest.ThrusterEstimateUpdate = 1;%Update the estimated ECI state in case thusters have been used in the previous time step
ObserverTest.version = 801;
...
ObserverTest.GPSGaussianCovariance  = [2; 2; 2; 0.001; 0.001; 0.001]*1e-3;% [15; 10; 5; 0.05; 0.04; 0.02]*1e-3;% GPD error 1-sigma covariance (ref. mail 20190311 from Nardecchia Luca)
...
ObserverTest.N_Wsigma_range = 10; % [10 6 4 2] %[10000  6000 3000]%[40 100 250]% [100 50 10];% [ 30000 20000 10000 6000 3000 1000 500 200] %10000
ObserverTest.Weight_GPSall = 4;% 30%
...
ObserverTest.RPYbetweenMagSensors = [45,-15,0]*pi/180; %RPY angles between the two magnetic sensors
...
ObserverTest.EulerAngleNoiseOnMagSigma = 0.01; %sigma on Magnetic measures as RPY angle (sigma in radiant)
...
ObserverTest.attitude_Km = 3; %gain on the first magnetic measure
ObserverTest.attitude_Kmbis = 3; %gain on the second magnetic measure
ObserverTest.attitude_Ka = 1; %gain on the accelerometer
ObserverTest.attitude_Kp = 1;
ObserverTest.attitude_Ki = 0.0; %gain on the gyros bias
ObserverTest.attitude_Komega = 0.5;
...
%saving all parameters to perform different iterations

save('Observer/ObserverTest.mat','ObserverTest');
disp('ObserverTest.mat file created ')
