...
% This method computes cost function, gradient and hessian for a single
% agent. Used to implement a decentralized simulation of the fleet. 
% Input: 
%   1) x_hat: (1 x 3) array containing the current position estimate 
%   2) Chi: (nagent x 3) matrix containing the a priori estimate for all
%   the agents.
%   3) GPS: (1 x 3) matrix containing the GPS measures for the agent
%   considered.
...
function out = Cj_num_v7_dec(x_hat, Chi, GPS, adjmat, j_select, nagent, weights, expval)
...
%  create first term J1
    Juwb = 0;
    for i=1:nagent
        Juwb = Juwb + ( norm(x_hat - Chi(i,:)) - adjmat(i,j_select) )^rho_uwb;
    end
    Juwb = Wuwb*Juwb;

%  create second term J2
   Jgps = Wgps*norm(x_hat-GPS)^rho_gps;

%  create third term J3
   Jsigma = Wsigma*norm(x_hat-Chi(j_select,:))^rho_sigma;
...
for j = 1:3
   for i = 1:nagent
       T = (norm(x_hat - Chi(i,:)) - adjmat(i,j_select))*(x_hat(j)-Chi(i,j))/norm(x_hat - Chi(i,:));
       if ~isnan(T)
            grad_Juwb(j) = grad_Juwb(j) + T;
       end
   end
   grad_Juwb(j) = 2*Wuwb*grad_Juwb(j);
   grad_Jgps(j) = 2*Wgps*(x_hat(j)-GPS(j));
   grad_Jsigma(j) = 2*Wsigma*(x_hat(j)-Chi(j_select,j));
end
...
for h = 1:3
   ...
   for k = 1:3
      for i = 1:nagent
        % first term
        T11 = (x_hat(h)-Chi(i,h))*(x_hat(k)-Chi(i,k))/norm(x_hat-Chi(i,:));
        if h == k
            T12 =  norm(x_hat-Chi(i,:)) - adjmat(i,j_select);
        else
            T12 = 0;
        end
        T1 = norm(x_hat-Chi(i,:))*(T11 + T12);     
        % second term
        T2 = (norm(x_hat-Chi(i,:)) - adjmat(i,j_select))*(x_hat(h)-Chi(i,h))*(x_hat(k)-Chi(i,k))/norm(x_hat-Chi(i,:));        
        % combine two terms
        T = (T1 - T2)/(norm(x_hat-Chi(i,:)))^2;
        if ~isnan(T)
