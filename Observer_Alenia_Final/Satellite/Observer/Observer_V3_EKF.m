function [satellites_iner_ECI_estimated,...
               ...
               satellites_attitude_estimated] = Observer_V3_EKF(satellites_iner_ECI, deputy_rel_LVLH, satellites_attitude, params,i,tspan)
...
% use structs with all the parameters 
global ObserverTest Agent map
...
%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    while (iteration <= ObserverTest.OptimizationsEachSampling)
        ...
        %%%%%%% UNCOMMENT TIC-TOC FOR COMPUTATIONAL TIME ANALSYS %%%%%%%    
%         tic
        % GPS OPTIMIZATION
        if ObserverTest.GPSopt_flag == 1
...
%         toc
        ...
        % KALMAN FILTERING
%         tic
        if ObserverTest.KF_flag == 1
            if strcmp(ObserverTest.KF_pos,'EKF') == 1
                ObserverTest.EKF_attitude = 0;
                for k=1:ObserverTest.Nagents
                    EKF_position_V1(params,i,tspan,k); %UKF estimator for the satellites position
                end
            elseif strcmp(ObserverTest.KF_pos,'UKF') == 1
                Position_UKF_V1_3;
            else
                disp('Wrong position observer selected');
                return
            end
        elseif ObserverTest.KF_flag == 0 && ObserverTest.GPSopt_flag == 1
            for k = 1:ObserverTest.Nagents
                Agent(k).xHatUKF(:,i+1) = [Agent(k).GPSopt(:,i+1); Agent(k).GPSpeedOpt(:,i+1)];
            end        
        end
%         toc
...
if(ObserverTest.AttitudeObserverOn==1)    
    for k=1:ObserverTest.Nagents
        ObserverTest.EKF_attitude = 1;
        tspan_att = tspan; %THEY SHOULD BE THE SAME
        Attitude_ObserverOpt_V2_EKF(map, params,i,tspan_att,k);
    ...
    % set direction matrix anyway
    for k = 1:ObserverTest.Nagents
        tspan_att = tspan;
        %%% Apriori Estimated Quaternion Dynamics as in Mahony 10.1109/TAC.2008.923738
        xstart = Agent(k).attitude_xHatUKF(:,i);
        xhat =  rk4_V1_1_decentralized(@AttitudeDynamics_V1_2_decentralized, tspan_att, xstart, params, k);
        % estimation assignment
        Agent(k).attitude(:,i+1) = xhat(1:7,end);
        
        % reference transformation and normalization (ref 5.3)
        Agent(k).R_ECI2Body = store_dcm(Agent(k).attitude(:,i+1));
    end
end
