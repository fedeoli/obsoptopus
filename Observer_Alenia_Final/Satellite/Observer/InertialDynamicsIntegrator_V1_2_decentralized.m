function dx = InertialDynamicsIntegrator_V1_2_decentralized(stato, params,ith_satellite,k_microstep)
...
dx(2) = stato(5);
dx(3) = stato(6);
dx(4) = fn_ECI(1) + fd_ECI(1) + fj2_ECI(1) + u_ECI(1)*ObserverTest.ControlOn + ObserverTest.CSI_jth(7)/ObserverTest.time_step;
dx(5) = fn_ECI(2) + fd_ECI(2) + fj2_ECI(2) + u_ECI(2)*ObserverTest.ControlOn + ObserverTest.CSI_jth(8)/ObserverTest.time_step;
dx(6) = fn_ECI(3) + fd_ECI(3) + fj2_ECI(3) + u_ECI(3)*ObserverTest.ControlOn + ObserverTest.CSI_jth(9)/ObserverTest.time_step;
end