...
while( k <= ObserverTest.Nagents && SkipChiefDidAll ==0);
    ...
    if((ObserverTest.StartSoonUWB == 1 || ((norm(myGPS_obs - Agent(k).xHatUKF(1:3,i+1)) < ObserverTest.UWBonGPSerror && i>ObserverTest.UWBOptimizationNoBeforeThan)) && ObserverTest.UWBoptimizationOn) )
        ...
        %X0_start_Kth_agent = myGPS;
        ObserverTest.OptimizationCycles.count = 0;
        [NewGPS, Jobs] = fminunc(@ObsUWBoptimization_v10,ObserverTest.X0_start_Kth_agent,ObserverTest.optionsObs);
        %disp(['Ottimizzazione GPS, drone  ' num2str(k) ': Jstart = ' num2str(ObserverTest.OptimizationCycles.Initial) ...
         %  ', Jfinal/Jstart = ' num2str(ObserverTest.OptimizationCycles.End/ObserverTest.OptimizationCycles.Initial)]);
