%% cost function
function [Jtot,grad,DynOpt] = cost_function_v11(x,params,DynOpt)
...
for j=1:n_iter
    ...
    %%%%% SPRING LIKE TERM %%%%%%
    K = zeros(size(x));
    K(end-(DynOpt.nparams-1):end) = DynOpt.scale_factor(1,5,1)*1;

    J_spr = K'*abs(DynOpt.temp_x0 - x);
    Jtot = Jtot + J_spr;

    DynOpt.J_spr_buf = J_spr;
