...
for k=1:length(DynOpt.time)
    ...
    if  ((distance < DynOpt.Nts) || (DynOpt.dJ_cond < DynOpt.dJcond_thresh)) && DynOpt.distance_safe_flag
        ...
        if DynOpt.input_tuning == 0
            ...
            disp(['INIT: Au: ', num2str(DynOpt.X_init(end-2)), ' d: ', num2str(DynOpt.X_init(end-1)), ' f: ', num2str(DynOpt.X_init(end))])
            disp(['CURRENT: Au: ', num2str(DynOpt.X(end-2)), ' d: ', num2str(DynOpt.X(end-1)), ' f: ', num2str(DynOpt.X(end))])
        end
                ...
                if DynOpt.distance_safe_flag == 1 || DynOpt.always_opt == 1
                    ...
                    try
                        ...
                        if DynOpt.input_tuning == 0
                            ...
                            if DynOpt.fcon_flag == 0 
                                [DynOpt.theta_u, J, DynOpt.exitflag] = DynOpt.fmin(@(x)DynOpt.cost_function(x,params_local),input_state,DynOpt.myoptioptions);
