function [satellites_iner_ECI_estimated,...
               ...
               satellites_attitude_estimated] = Observer_V2_3(satellites_iner_ECI, deputy_rel_LVLH, satellites_attitude, params,i,tspan)
...
deputy_rel_LVLH_estimated = satellites_attitude;
satellites_attitude_estimated = deputy_rel_LVLH;
...
if(ObserverTest.ObserverON)
    ...
    for k = 1:ObserverTest.Nagents,
        ...
        X = rk4_V1_1_decentralized(@InertialDynamicsIntegrator_V1_1_decentralized, tspan, Agent(k).xHatUKF(:,i), params,k);
...
if(ObserverTest.AttitudeObserverOn==1)
    ...
    for k=1:ObserverTest.Nagents,
        ...
        %estimation error
        Agent(k).attitude_error(1:4,i+1) = quatnormalize(quatmultiply(quatinv(Agent(k).attitude(1:4,i+1)'), Agent(k).attitude_xHatUKF(1:4,i+1)')); %by CHALLA (8) but we can also swap the inverse among vectors in the quatmultiply to have the standard true-estimate
        Agent(k).attitude_error(5:7,i+1) = Agent(k).attitude(5:7,i+1) - Agent(k).attitude_xHatUKF(5:7,i+1) ;
        Agent(k).magyrobias_error(:,i+1) = [ObserverTest.MagnetoBias;ObserverTest.GyroBias] - Agent(k).attitude_xHatUKF(8:end,i+1);
