function [u_out, satellites_iner_ECI, params] = SingleAxisThruster_V2_2obs(t, u, satellites_iner_ECI, deputy_rel_LVLH, satellites_attitude, params)

global ObserverTest Agent

%   SingleAxisThruster_V2_1.m
%   Made by Sapienza Gn Lab + CHANGES to also modify the estimated
%   quantities by Tor Vergata
%
...
elseif strcmpi(params.ControlStrategy,  'Impulsive')
    ...
    if (params.check_attitude == 1) && (params.Burn_executed == 0)
        ...
        for j = 1:N_deputy
            ...
            %OBSERVER: ESTIMATE STATE UPDATE %THIS SHOULD (WILL) BE DONE WITHIN THE APRIORI STEP OF THE UKF
            ObserverTest.deputy_rel_LVLH_estimated(:,j) = ObserverTest.deputy_rel_LVLH_estimated(:,j) + [zeros(3,1); DV_applied];
            chief_coe_estimated = rv2coe_V1_1(ObserverTest.satellites_iner_ECI_estimated(1:3), ObserverTest.satellites_iner_ECI_estimated(4:6), mi);
            ObserverTest.satellites_iner_ECI_estimated(1 + 6*j : 6 + 6*j, 1) = rel2iner_V1_1(ObserverTest.deputy_rel_LVLH_estimated(:,j)', ObserverTest.satellites_iner_ECI_estimated(1:6)', chief_coe_estimated, params);
            Agent(j+1).xHatUKF(:,ObserverTest.u_time_index+1) = ObserverTest.satellites_iner_ECI_estimated(1 + 6*j : 6 + 6*j,1);
