...
struct.w = 5;
struct.Nts = 3;
struct.dJcond_thresh = 1e-3;                                                  % the optimisation is run if COND > THRESH (set to 0 to set a nocare condition)
struct.theta = 0;
struct.beta = 0;
struct.gamma = 1;
...
struct.rand_init = 0;
% measures
struct.noise_amp = 1*struct.noise_enable*[0*1e-4*ones(3,1); 0*ones(3*struct.nMagneto,1)];
%%%% STATE %%%%
if struct.integration_pos == 1 && struct.integration_att == 1
    % POS + ATT
    struct.init_error_amp = 1*struct.noise_enable*[0*ones(3,1); 0*ones(3,1); 1*ones(4,1); 0.5*ones(3,1)];
elseif struct.integration_pos == 1 && struct.integration_att == 0
    ...
    struct.init_error_amp = 1*struct.noise_enable*[1*ones(4,1); 0.5*ones(3,1)];
end
%%%% PARAMS %%%%
struct.ParamsAllPos = 1;
struct.init_param_error_amp = 0*struct.noise_enable*ones(1,struct.nparams)*50;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
