function [DynOpt, params] = MainOpt_DEZ_general_v20_fun_params_alenia(struct)
...
if DynOpt.ObserverOn == 1
    ...
    param_init = (1+1e-2*DynOpt.init_param_error_amp.*randn(length(DynOpt.param_estimate),1)).*DynOpt.Xtrue(DynOpt.StateDim+1:end);
    ...
    DynOpt.get_measure = @get_measure_v2;
    DynOpt.cost_function = @cost_function_v2;
    DynOpt.get_measure_name = 'get_measure_v2';
...
%% Observer implementation 
    if DynOpt.OptimisationOn
        ...
        for k=1:length(DynOpt.time)
            ...
            if(k>1)
                ...
                % save the desired attitude and input
                DynOpt.desatt_true(:,k) = params.DesiredAttitude;
            end
            ...
            if DynOpt.simulationModel == 1
                [DynOpt.buf_dy, Y_true] = DynOpt.get_measure(DynOpt.Xtrue,0,measure_forward,DynOpt.buf_dy,params);
            ...
            if  ((distance < DynOpt.Nts) || (DynOpt.dJ_cond < DynOpt.dJcond_thresh)) && DynOpt.distance_safe_flag
                ...
                % measures
                [DynOpt.buf_dyhat, Yhat] = DynOpt.get_measure(DynOpt.OptXstory(:,DynOpt.ActualTimeIndex),0,measure_forward,DynOpt.buf_dyhat,params);
                ...
                if (k < max(1,DynOpt.WindowSamples)) 
                    ...
                    if(DynOpt.ForwardOptimization == 1) 
                        ...
                        if ( (J_diff <= DynOpt.Jdot_thresh) || (distance > DynOpt.safety_interval) )  || DynOpt.blue_flag
                            ...
                            % performed 
                            [DynOpt.buf_dyhat_temp, Yhat] = DynOpt.get_measure(x_propagate,0,measure_forward,DynOpt.buf_dyhat_temp,params);
    ...
    if DynOpt.OptimisationOn == 1
        DynOpt.Y_int = zeros(3,DynOpt.Niter);
        DynOpt.Yhat_int = zeros(3,DynOpt.Niter);
        for i=1:3
    ...
    DynOpt.Opt_quat = wrapTo2Pi(RotationConversion_V2_1('QtoEA321', DynOpt.OptXstory(1:4,:)')*pi/180)';
