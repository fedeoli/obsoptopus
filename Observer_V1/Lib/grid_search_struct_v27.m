...
function grid_search_struct(window_interval,window_step,sample_interval,sample_step,pathname)
    ...
    for counter_window = window_interval(1):window_step:window_interval(2)
        for counter_sample = sample_interval(1):sample_step:sample_interval(2)
            ...
            if ~isfile(filename)
                struct = init_struct_analyse_v2(counter_window,counter_sample);

                [DynOpt, params] = MainOpt_DEZ_general_v12_fun_params(struct);
                save(final_path);
                keep path counter_window window_interval window step counter_sample sample_interval sample_step
                load temp
            else
                delete(filename)
