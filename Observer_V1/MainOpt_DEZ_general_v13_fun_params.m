function [DynOpt, params] = MainOpt_DEZ_general_v13_fun_params(struct)
    ...
    for k=1:DynOpt.Niter
        ...
        if(k>1)
            ...
            if strcmp(DynOpt.int_flag,'rk4') 
                ...
                temp = rk4_V1_1(DynOpt.model, tspan, DynOpt.X, params);
                DynOpt.X = temp(:,end);
                    ...
                    if strcmp(DynOpt.opt_method,'default')
                        try 
                            if DynOpt.fcon_flag == 0
                                [NewXopt, J, DynOpt.exitflag] = fminsearch(DynOpt.cost_function,DynOpt.temp_x0,myoptioptions);
                            else
                        ...
                        if ((abs(J_dot) > DynOpt.J_thresh) || (state_diff < DynOpt.diff_thresh) || DynOpt.blue_flag)
                            ...
                            if DynOpt.BackTimeIndex < DynOpt.Niter
                                for j =1:DynOpt.WindowSamples-1     
                                    set_input(DynOpt.BackTimeIndex+j);

                                    if strcmp(DynOpt.int_flag,'rk4') 
                                        tspan = [DynOpt.time(DynOpt.BackTimeIndex+j), DynOpt.time(DynOpt.BackTimeIndex+j)+DynOpt.Ts];
