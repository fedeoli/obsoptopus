%% ONLY ANALYTICAL TEST
function Jtot = cost_function_v1(x)
...
for j=1:DynOpt.w
    ...
    if(DynOpt.ForwardOptimization ~= 1) %backward        
        ...
        n_der = DynOpt.dim_out-DynOpt.n_sensor;
        for i=1:n_der
    ...
    pen_state = DynOpt.pen_state*(norm(DynOpt.temp_x0(1:DynOpt.StateDim)-X(1:DynOpt.StateDim)));
    pen_param = DynOpt.pen_param*norm(X(DynOpt.StateDim+1:end)-DynOpt.temp_x0(DynOpt.StateDim+1:end));
    Jtot = DynOpt.mag_factor*(Jtot + pen_state + pen_param);
